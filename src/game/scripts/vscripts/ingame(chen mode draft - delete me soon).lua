...
function Ingame:handleRespawnModifier()
    ...
    if not hero:IsControllableByAnyPlayer() and not hero:IsHero() then
        local attacker = nil
        ...
        if attacker and attacker:GetPlayerOwner()  then
            ...
            --print( "team: " .. GetTeamName( team ) .. "; playerID: " .. playerID )
            
            local unitName = hero:GetUnitName()
            
           -- if unitName == "npc_dota_roshan" then
           --     unitName = "npc_dota_roshan_fake"
            --end
            
            local unit = CreateUnitByName( unitName, hero:GetAbsOrigin(), true, owner, owner, team )
            
            unit:SetControllableByPlayer( playerID, false )
            unit:SetOwner( owner )

            print(unit:GetModelName())
            if unit:GetModelName() == "models/creeps/lane_creeps/creep_bad_melee/creep_bad_melee.vmdl" then
                unit:SetModel("models/creeps/lane_creeps/creep_bad_melee/creep_bad_melee_mega.vmdl")
                unit:SetOriginalModel("models/creeps/lane_creeps/creep_bad_melee/creep_bad_melee_mega.vmdl")
            elseif unit:GetModelName() == "models/creeps/lane_creeps/creep_bad_ranged/lane_dire_ranged.vmdl" then
                unit:SetModel("models/creeps/lane_creeps/creep_bad_ranged/lane_dire_ranged_mega.vmdl")
                unit:SetOriginalModel("models/creeps/lane_creeps/creep_bad_ranged/lane_dire_ranged_mega.vmdl")
            elseif unit:GetModelName() == "models/creeps/lane_creeps/creep_radiant_melee/radiant_melee.vmdl" then
                unit:SetModel("models/creeps/lane_creeps/creep_radiant_melee/radiant_melee_mega.vmdl")
                unit:SetOriginalModel("models/creeps/lane_creeps/creep_radiant_melee/radiant_melee_mega.vmdl")
            elseif unit:GetModelName() == "models/creeps/lane_creeps/creep_radiant_ranged/radiant_ranged.vmdl" then
                unit:SetModel("models/creeps/lane_creeps/creep_radiant_ranged/radiant_ranged_mega.vmdl")
                unit:SetOriginalModel("models/creeps/lane_creeps/creep_radiant_ranged/radiant_ranged_mega.vmdl")
            end
            
            unit:SetForwardVector( hero:GetForwardVector() )
            attacker:EmitSound( "Hero_Chen.HolyPersuasionCast" )
            unit:EmitSound( "Hero_Chen.HolyPersuasionEnemy" )

            if not util:isPlayerBot(attacker:GetPlayerOwner():GetPlayerID()) then
                if attacker:IsRealHero() then 
                   unit:AddNewModifier(attacker, nil, "modifier_kill", {duration = 60})
                else
                   unit:AddNewModifier(attacker, nil, "modifier_kill", {duration = 20}) 
                end
            end

            local part = ParticleManager:CreateParticle( "particles/units/heroes/hero_chen/chen_holy_persuasion.vpcf", PATTACH_ABSORIGIN, unit )
        
            ParticleManager:SetParticleControlEnt( part, 1, attacker, PATTACH_ABSORIGIN, "", attacker:GetAbsOrigin(), true )
            ParticleManager:SetParticleControl( part, 2, Vector( 0.1, 0, 0 ) )
