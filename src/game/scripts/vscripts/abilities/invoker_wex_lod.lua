	...
	for i=1, 3, 1 do
		if keys.caster.invoked_orbs[i] ~= nil then
			...
			if orb_name == "invoker_quas_lod" then
				local quas_ability = keys.caster:FindAbilityByName("invoker_quas_lod")
				if quas_ability ~= nil then
					quas_ability:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_invoker_quas_lod_instance", nil)
				end
