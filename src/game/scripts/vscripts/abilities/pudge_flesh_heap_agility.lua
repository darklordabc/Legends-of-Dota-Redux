...
function modifier_flesh_heap_agi:IsPassive()
    return true
end
...
function modifier_flesh_heap_agi:GetFleshHeapKills()
  if self.nKills == nil then
    self.nKills = 0
  end
  return self.nKills
...
function modifier_flesh_heap_agi:OnCreated( kv )
  ...
  self.flesh_heap_agi_buff_amount = self:GetAbility():GetSpecialValueFor( "flesh_heap_agility_buff_amount" ) or 0
  if IsServer() then
...
function modifier_flesh_heap_agi:OnDeath(keys)
