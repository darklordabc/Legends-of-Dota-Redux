...
if spell_lab_survivor_attack_range_modifier == nil then
	spell_lab_survivor_attack_range_modifier = class({})
end
...
function spell_lab_survivor_attack_range_modifier:OnDeath(kv)
  if IsServer() then
...
function spell_lab_survivor_attack_range_modifier:IsHidden()
	return self:GetStackCount() < 1
end

function spell_lab_survivor_attack_range_modifier:AllowIllusionDuplicate ()
  return true
end
function spell_lab_survivor_attack_range_modifier:IsPurgable()
	return false
end

function spell_lab_survivor_attack_range_modifier:OnCreated()
	if IsServer() then
		self.lastdeath = GameRules:GetGameTime()
		if not self:GetParent():IsRealHero() then
  local hOwner = self:GetParent():GetOwner()
  if hOwner ~= nil then
    local hOriginModifier = hOwner:GetAssignedHero():FindModifierByName("spell_lab_survivor_attack_range_modifier")
...
function spell_lab_survivor_attack_range_modifier:OnIntervalThink()
	if IsServer() then
		if not self:GetParent():IsRealHero() then
			self:StartIntervalThink( -1 )
			return
...
function spell_lab_survivor_attack_range_modifier:GetAttributes()
