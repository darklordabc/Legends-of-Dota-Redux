...
modifier_flying_with_slow = {}
modifier_flying_with_slow.__index = modifier_flying_with_slow
function modifier_flying_with_slow.new(construct, ...)
    local instance = setmetatable({}, modifier_flying_with_slow)
    if construct and modifier_flying_with_slow.constructor then modifier_flying_with_slow.constructor(instance, ...) end
    return instance
end
...
function modifier_flying_with_slow.GetModifierMoveSpeedBonus_Percentage(self)
    ...
    if self:GetStackCount()==0 then
        local ms = TS_ITE(self:GetAbility(),function() return -self:GetAbility():GetSpecialValueFor("flying_movespeed_slow_pct") end,function() return -50 end)
