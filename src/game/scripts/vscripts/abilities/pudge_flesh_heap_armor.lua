...
--[[function pudge_flesh_heap_armor:OnHeroDiedNearby( hVictim, hKiller, kv )
  ...
  if hVictim:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and self:GetCaster():IsAlive() then
    ...
    if hKiller == self:GetCaster() or self.fleshHeapRange >= flDistance then
      ...
      local hBuff = self:GetCaster():FindModifierByName( "modifier_flesh_heap_armor" )
      if hBuff ~= nil then
        hBuff:SetStackCount( self.nKills )
        self:GetCaster():CalculateStatBonus()
      else
        self:GetCaster():AddNewModifier( self:GetCaster(), self,  "modifier_flesh_heap_armor" , {} )
      end
...
function modifier_flesh_heap_armor:IsPassive()
    return true
end
...
function modifier_flesh_heap_armor:GetFleshHeapKills()
  if self.nKills == nil then
    self.nKills = 0
  end
  return self.nKills
...
function modifier_flesh_heap_armor:OnCreated( kv )
  if not self:GetAbility() then
    self:GetParent():RemoveModifierByName("modifier_flesh_heap_armor")
...
function modifier_flesh_heap_armor:OnRefresh( kv )
  if not self:GetAbility() then
    self:GetParent():RemoveModifierByName("modifier_flesh_heap_armor")
    self:GetParent():CalculateStatBonus()
    return
...
function modifier_flesh_heap_armor:OnDeath(keys)
