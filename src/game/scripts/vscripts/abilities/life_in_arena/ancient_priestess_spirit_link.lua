...
function ancient_priestess_spirit_link:OnSpellStart() 
	...
	local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_lone_druid/lone_druid_spiritlink_cast.vpcf", PATTACH_WORLDORIGIN, caster)
	...
	Timers:CreateTimer(0.1,
		function()
			...
			if target and not PlayerResource:IsDisableHelpSetForPlayerID(target:GetPlayerOwnerID(),caster:GetPlayerOwnerID()) then 
				...
				local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_lone_druid/lone_druid_spiritlink_cast.vpcf", PATTACH_WORLDORIGIN, prevTarget)
