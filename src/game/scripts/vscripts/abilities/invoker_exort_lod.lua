...
function invoker_replace_orb_modifiers(keys)
	...
	while keys.caster:HasModifier("modifier_invoker_quas_lod_instance") do
		keys.caster:RemoveModifierByName("modifier_invoker_quas_lod_instance")
	end
	while keys.caster:HasModifier("modifier_invoker_wex_lod_instance") do
		keys.caster:RemoveModifierByName("modifier_invoker_wex_lod_instance")
	end
	while keys.caster:HasModifier("modifier_invoker_exort_lod_instance") do
		keys.caster:RemoveModifierByName("modifier_invoker_exort_lod_instance")
	end
	...
	for i=1, 3, 1 do
		if keys.caster.invoked_orbs[i] ~= nil then
			...
			if orb_name == "invoker_quas_lod" then
				local exort_ability = keys.caster:FindAbilityByName("invoker_quas_lod")
				if exort_ability ~= nil then
					exort_ability:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_invoker_quas_lod_instance", nil)
				end
			elseif orb_name == "invoker_wex_lod" then
				local wex_ability = keys.caster:FindAbilityByName("invoker_wex_lod")
				if wex_ability ~= nil then
					wex_ability:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_invoker_wex_lod_instance", nil)
				end
			elseif orb_name == "invoker_exort_lod" then
				local exort_ability = keys.caster:FindAbilityByName("invoker_exort_lod")
				if exort_ability ~= nil then
					exort_ability:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_invoker_exort_lod_instance", nil)
				end
			end
		end
	end
