...
----------    ENFEEBLE    ------------
--------------------------------------
LinkLuaModifier("modifier_imba_enfeeble_debuff", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_enfeeble_debuff_vision_handler", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
...
---------    BRAIN SAP     -----------
--------------------------------------
LinkLuaModifier("modifier_imba_brain_sap_mana", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_brain_sap_baby_bane", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_imba_nightmare_dot", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_nightmare_invul", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_nightmare_vision", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_nightmare_talent", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
...
-----       FIEND'S GRIP         -----
--------------------------------------
LinkLuaModifier("modifier_imba_fiends_grip_handler", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fiends_grip_demon", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fiends_grip_talent", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
	...
	if  IsServer() then
		...
		if enfeeble_debuff and not self.baby then
			...
			if #stacks_table > 0 then
				...
				-- Spawn daemon
				--[[Elfansoer: temporary hot fix to unknown reason `demon` return false]]
				...
				local demon	=	CreateUnitByName("npc_dota_techies_minefield_sign", caster:GetAbsOrigin() + RandomVector(100), true, caster, caster, caster:GetTeam() )
				parent.demons = parent.demons or {}
				parent.demons[demon] = true
	...
	if IsServer() then
		...
		if not self.baby then
			...
			if parent:IsMagicImmune() or self.propogated == 1 or  not parent:IsAlive() then
				...
				--[[Elfansoer: fixed fiends grip sound not ended on Lotus]]
				StopSoundOn("Hero_Bane.FiendsGrip.Cast", caster)
				--[[Elfansoer: temporary hot fix to unknown reason `demon` return false above]]
				if parent.demons then
					for demon,_ in pairs(parent.demons) do
						UTIL_Remove( demon )
					end
				end
			end
...
LinkLuaModifier("modifier_special_bonus_imba_bane_2", "abilities/dota_imba/hero_bane", LUA_MODIFIER_MOTION_NONE)
