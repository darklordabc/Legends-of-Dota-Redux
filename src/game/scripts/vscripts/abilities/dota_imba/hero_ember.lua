...
LinkLuaModifier("modifier_imba_flame_guard_passive", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_sleight_of_fist_caster", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_sleight_of_fist_marker", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_searing_chains_attack", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_searing_chains_debuff", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fire_remnant_state", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fire_remnant_charges", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fire_remnant_cooldown", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fire_remnant_dash", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fire_remnant_timer", "abilities/dota_imba/hero_ember.lua", LUA_MODIFIER_MOTION_NONE)

--------------------------------------------------------------------------------

-- Finds all remnants in the map
function FindActiveRemnants(caster)
	-- elfansoer: fix remnants not found
	...
	-- 	if not remnant:HasModifier("modifier_imba_fire_remnant_state") then
	...
	for _,unit in pairs(units) do
		if unit:HasModifier( "modifier_imba_fire_remnant_state" ) then
			table.insert( available_remnants, unit )
		end
	end

	if #available_remnants>0 then
		return available_remnants
	else
		return nil
	end
...
function modifier_imba_fire_remnant_state:OnCreated(keys)
	if IsServer() then
		-- elfansoer: fix model missing
		self:GetParent():SetModel( self:GetCaster():GetModelName() )
		self:GetParent():SetOriginalModel( self:GetCaster():GetModelName() )
		self:GetParent():StartGesture( ACT_DOTA_VERSUS )
		local vector = self:GetParent():GetOrigin()-self:GetCaster():GetOrigin()
		vector.z = 0
...
function imba_ember_spirit_fire_remnant:OnSpellStart()
	if IsServer() then
		...
		if charges_modifier:GetStackCount() > 0 then
			...
			-- Elfansoer: Fix missing reference to imba unit (uses default instead)
			-- local remnant = CreateUnitByName("npc_imba_ember_spirit_remnant", target_loc, true, caster, caster, caster:GetTeamNumber())
			local remnant = CreateUnitByName("npc_dota_ember_spirit_remnant", target_loc, true, caster, caster, caster:GetTeamNumber())
