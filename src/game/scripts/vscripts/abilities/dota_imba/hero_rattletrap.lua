...
LinkLuaModifier("modifier_imba_rattletrap_battery_assault_percussive_maint", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_rattletrap_power_cogs", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_rattletrap_cog_push", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_imba_rattletrap_power_cogs_charge_coil_counter", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_rattletrap_power_cogs_charge_coil_instance", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_rattletrap_power_cogs_rotational", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_HORIZONTAL)

LinkLuaModifier("modifier_imba_rattletrap_rocket_flare_critical", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_rattletrap_hookshot", "abilities/dota_imba/hero_rattletrap", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function imba_rattletrap_power_cogs:OnSpellStart()
	...
	-- Elfansoer: fix enemies with spell immunity stuck in cogs
	...
	local units = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, self:GetSpecialValueFor("cogs_radius"), DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
	...
	for _, unit in pairs(units) do
		...
		if unit:IsMagicImmune() or unit:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
...
function modifier_imba_rattletrap_cog_push:OnDestroy()
	...
	-- Elfansoer: Fix power cogs damage not putting cd on Heart or Blink
	local damageTable = {
