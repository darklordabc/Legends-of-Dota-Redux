...
function isPatron(mutator_name) {
	...
	if (patrons) {
		for (var patron in patrons) {
			if (
				patrons[patron].steamID3 == util.getSteamID32() ||
				patrons[patron].steamID64 == Game.GetPlayerInfo(Game.GetLocalPlayerID()).player_steamid
			) {
				return true;
				break;
			}
...
function OnSelectedSkillsChanged(table_name, key, data) {
	...
	if (playerID == Players.GetLocalPlayer()) {
		...
		var isBelowThreshold = enableAlternativeThreshold
			? function (ability) {
	...
	if (key.match("booster") != null && Players.GetLocalPlayer() == parseInt(key.replace(key.match("booster"), ""))) {
		...
		if (isBoosterDraftGamemode()) {
			...
			if (!boosterDraftInitiated && !data.boosterDraftDone) {
				...
				$("#boosterDraftBoosters")
					.Children()
					[$("#boosterDraftBoosters").Children().length - 1].SetHasClass("current", true);
				...
				for (var k in boosters) {
					boosters[k].FindChildTraverse("booster").style.hueRotation =
						Math.floor((360 / players) * i) + "deg;";
					i++;
				}
...
function fixBacktrackUI() {
	...
	if (masterRoot != null) {
		masterRoot.SetHasClass(
			"phase_option_selection_selected",
			selectedPhase == PHASE_OPTION_SELECTION || util.reviewOptions,
		);
		masterRoot.SetHasClass("review_selection", util.reviewOptions);
		masterRoot.SetHasClass(
			"phase_selection_selected",
	...
	function doInsertHeroes(container, heroList) {
		...
		for (var i = 0; i < heroList.length; ++i) {
			(function () {
				...
				newPanel.BLoadLayout(
					"file://{resources}/layout/custom_game/game_setup/game_setup_hero.xml",
					false,
					false,
				);
...
function highlightDropSlots() {
	...
	for (var i = 1; i <= 6; ++i) {
		...
		if (alreadyHas) {
			...
			ab.SetHasClass(
				"lodSelectedDrop",
				easyAdd || (ourBuild[i] != null && isUlt == isUltimateAbility(ourBuild[i])),
			);
		}
	}
...
function makeSkillSelectable(abcon) {
	...
	abcon.SetPanelEvent("onactivate", function () {
		...
		if (heroOwner != null) {
			if (currentPhase != PHASE_BANNING) setSelectedHelperHero(heroOwner, true);
		}
	});
		...
		calculateHeroFilters = function () {
			...
			for (var heroName in heroPanelMap) {
				...
				if (shouldShow && heroFilterInfo.classType) {
					...
					if (info) {
						if (
							(info.AttackCapabilities == "DOTA_UNIT_CAP_MELEE_ATTACK" &&
								heroFilterInfo.classType == "ranged") ||
							(info.AttackCapabilities == "DOTA_UNIT_CAP_RANGED_ATTACK" &&
...
function getSkillFilterInfo(abilityName) {
	...
	if (heroName != null && heroData[heroName] != null && heroData[heroName].HeroPerk != null) {
		...
		if (
			heroPerks.indexOf(abilityName) == -1 &&
			abilityPerks.every(function (v) {
				return heroPerks.indexOf(v) == -1;
			})
		) {
	...
	if (shouldShow && searchText.length > 0) {
		...
		for (var i = 0; i < searchParts.length; ++i) {
			...
			if (
				localAbName.indexOf(prt) == -1 &&
				localOwningHeroName.indexOf(prt) == -1 &&
				abilityName.indexOf(prt) == -1
			) {
				shouldShow = false;
				break;
			}
...
function OnSkillTabShown(tabName) {
	if (firstSkillTabCall) {
						...
						if (useSmartGrouping && !activeTabs["mostused"]) {
							...
							if (groupKey) {
								...
								if (groupCon == null) {
									...
									groupCon.SetHasClass(
										"draftSkills",
										isDraftGamemode() && currentPhase != PHASE_BANNING,
									);
								}
        ...
        */
		...
		var tabList = ["main", "neutral", "custom", "imba", "superop", "mostused"];
				...
				tabButton.SetPanelEvent("onactivate", function () {
					...
					if (GameUI.IsControlDown() && tabName != "mostused" && !activeTabs["mostused"]) {
						...
						for (var g in abilityStore) {
							abilityStore[g].SetHasClass(
								"lodDraftAbility",
								tabName == "mostused" || (isDraftGamemode() && currentPhase == PHASE_SELECTION),
							);
						}
					}
...
function addPlayerToTeam(playerID, panel, reviewContainer, shouldMakeSmall) {
    ...
    */
	...
	if (newPlayerPanel == null) {
		...
		if (/radiant/i.test(reviewContainer.id)) newPlayerPanel.style.zIndex = 20 - playerID;
	} else {
		...
		mutators.forEach(function (item, i) {
			...
			function addImage(state) {
				if (!images[state]) {
					...
					optionMutatorImage.style.backgroundImage =
						"url('file://{images}/custom_game/mutators/mutator_" + state + ".png');";
				...
				if (returnString) {
					...
					for (var state in optionMutator.states) {
						if (typeof optionMutator.states[state] === "object") {
							...
							stateName = Object.keys(optionMutator.states).filter(function (key) {
								return optionMutator.states[key] === optionValueList[item.name];
							})[0];
				...
				for (var state in item.states) {
					if (typeof item.states[state] === "object") {
						...
						if (found) {
							if (item.states[Object.keys(item.states)[i - 1]] !== undefined) {
								...
								if (item.default !== undefined) {
									...
									nextItem =
										item.states[Object.keys(item.states)[Object.keys(item.states).length - 1]];
								}
							}
						} else {
						...
						if (item.states[Object.keys(item.states)[i - 1]] !== undefined) {
							...
							if (item.default !== undefined) {
								nextItem =
									item.default[Object.keys(item.default)[Object.keys(item.default).length - 1]];
							} else {
								nextItem = item.states[Object.keys(item.states)[Object.keys(item.states).length - 1]];
							}
						}
				...
				if (found) {
					...
					for (var state in mutatorList[field].states) {
						if (typeof mutatorList[field].states[state] === "object") {
							...
							stateName = Object.keys(mutatorList[field].states).filter(function (key) {
								return mutatorList[field].states[key] === optionValueList[field];
							})[0];
				...
				for (var i = 0; i < fieldData.length; ++i) {
					...
					(function () {
						...
						if (fieldData[i].name === "lodOptionGamemode") {
							...
							var infoLabel = $.CreatePanel(
								sort === "shopTree" ? "ToggleButton" : "Label",
								mainSlot,
							...
							if (patreon_options[fieldName]) {
								patreonMutators = CustomNetTables.GetTableValue("phase_pregame", "patreon_features")
									.Options;
								var mutatorOfTheDay = CustomNetTables.GetTableValue("phase_pregame", "mutatorOfTheDay")
									.value;
								var count_ = 0;
								var found_ = false;
							...
							var floatRightContiner = $.CreatePanel(
								"Panel",
								mainSlot,
								"option_panel_field_" + fieldName + "_container",
							);
							floatRightContiner.AddClass("optionsSlotPanelContainer");

							// Create stores for the newly created items
							var hostPanel;
							var slavePanel = $.CreatePanel(
								"Label",
								floatRightContiner,
								"option_panel_field_" + fieldName + "_slave",
							);
							...
							if (patreon_options[fieldName] && !isPatron(fieldName)) {
								...
								switch (sort) {
									...
									case "range":
										...
										hostPanel.BLoadLayout(
											"file://{resources}/layout/custom_game/slider.xml",
											false,
											false,
										);
										hostPanel.AddClass("optionsSlotPanelHost");
										...
										var onGetNewSliderValue = function (
											newValue,
											shouldNetwork,
											ignoreSlider,
											ignoreText,
...
function generateFormattedHeroStatsString(heroName, info) {
	...
	if (info != null) {
		...
		var startingAttributes =
			info.AttributeBaseStrength + info.AttributeBaseAgility + info.AttributeBaseIntelligence;
		var attributesPerLevel = stringToDecimalPlaces(
			info.AttributeStrengthGain + info.AttributeAgilityGain + info.AttributeIntelligenceGain,
		);
...
function isDraftGamemode() {
	...
	return (
		...
		netTableValue == 3 ||
		netTableValue == 6 ||
		optionValueList["lodOptionCommonGamemode"] == 5 ||
		optionValueList["lodOptionCommonGamemode"] == 3 ||
		optionValueList["lodOptionCommonGamemode"] == 6
	);
}
	...
	switch (key) {
		case "phase":
			...
			if (currentPhase == PHASE_OPTION_VOTING) {
				...
				if (mapName.match("classic")) $("#middleButtons").visible = false;
			}
			...
			if (currentPhase == PHASE_SELECTION) {
				...
				if (!Game.IsInToolsMode()) {
					if (!$.GetContextPanel().isSinglePlayer) {
						...
						$("#heroBuilderLockButton").SetHasClass(
							"pressed",
							!$("#heroBuilderLockButton").BHasClass("pressed"),
						);
					}
				}
...
function OnOptionChanged(table_name, key, data) {
	...
	switch (key) {
		...
		case "lodOptionAdvancedUniqueSkills":
			...
			updateHeroPreviewFilters();
			updateRecommendedBuildFilters();
			$("#mainSelectionRoot").SetHasClass(
				"unique_skills_mode",
				optionValueList["lodOptionAdvancedUniqueSkills"] > 0,
			);
			break;
		...
		case "lodOptionAdvancedUniqueHeroes":
			$("#mainSelectionRoot").SetHasClass(
				"unique_heroes_mode",
				optionValueList["lodOptionAdvancedUniqueHeroes"] == 1,
			);
			break;
...
function UpdateTimer() {
	...
	if (placeInto != null) {
		...
		var timeLeft =
			currentPhase == PHASE_INGAME
				? Math.ceil(Game.GetDOTATime(false, true))
				: Math.ceil(endOfTimer - Game.Time());
		...
		if (isBoosterDraftGamemode()) {
			try {
				$("#boosterDraftBoosters")
					.Children()
					[$("#boosterDraftBoosters").Children().length - 1].FindChildTraverse("lodBoosterPackLabel").text =
					placeInto.text;
			} catch (err) {}
		}
...
function gamemodesScroll(direction) {
	if ($("#gamemodesContainer").num == undefined) $("#gamemodesContainer").num = 0;

	var childCount = $("#scroller").GetChildCount();

	var dir = direction == "right" ? -1 : 1;
	if (childCount + $("#gamemodesContainer").num + dir == 0 || $("#gamemodesContainer").num + dir > 0) return;

	$("#gamemodesContainer").num += dir;
	$("#scroller").style.transform =
		"translateX(" +
		(($("#gamemodesContainer").num / childCount) * 100 * $("#scroller").actuallayoutwidth) /
			$("#gamemodesContainer").actuallayoutwidth +
		"%);";
}
...
function LoadOptionsHandler(data) {
	...
	if (content.lodOptionCommonDraftAbilities)
		setOption("lodOptionCommonDraftAbilities", content.lodOptionCommonDraftAbilities);
}
...
function addVotingOption(name) {
	...
	panel.UpdateVotes = function (info) {
		...
		$.Each(
			[panel.FindChildTraverse("voteCountNoPercentage"), panel.FindChildTraverse("voteCountYesPercentage")],
			function (countLabel, index) {
