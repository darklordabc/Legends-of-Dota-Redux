...
function addEmotion(parent, emotStr) {
	if (!emotions.hasOwnProperty(emotStr)) return;
...
function say() {
	var time = Game.Time();
	if (time < lastSendedMessageTime + sendTimeout && lastSendedMessageTime != 0) return;
...
function showChatMessage(args) {
	...
	if (args.player == -1) {
		...
		label.text =
			"(" +
			$.Localize(channels[args.channel].name) +
			") " +
			" " +
			Game.GetPlayerInfo(args.player).player_name +
			": " +
			msg;
	}
	label.style.color = channels[args.channel].color;

	if (matches) for (var i = 0; i < matches.length; i++) addEmotion(label.GetChild(i), matches[i]);
}
