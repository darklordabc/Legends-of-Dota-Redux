			...
			if (panel.BHasHoverStyle()) {
				...
				if (description != "command_menu_command_descr_" + data.title) {
					$.DispatchEvent(
						...
						panel.FindChildTraverse("commandTitle"),
						panel.FindChildTraverse("commandTitle").id,
						"file://{resources}/layout/custom_game/custom_text_tooltip.xml",
						"text=" + description,
					);
...
function createCommandGroup(data) {
	...
	groupHeader.SetPanelEvent("onactivate", function () {
		if (!groupHeader.checked) {
			...
			panel.FindChildTraverse("groupContents").style.height =
				panel.FindChildTraverse("groupContents").tempHeight + "px;";
		}

		if (currentMenu) {
			currentMenu.FindChildTraverse("groupHeader").checked = !currentMenu.FindChildTraverse("groupHeader")
				.checked;
			if (!currentMenu.FindChildTraverse("groupHeader").checked) {
				currentMenu.FindChildTraverse("groupContents").style.height = "0px;";
			} else {
				currentMenu.FindChildTraverse("groupContents").style.height =
					currentMenu.FindChildTraverse("groupContents").tempHeight + "px;";
			}
		}
